...
try:
    ...
    API_KEY = None  # Secrets 설정이 안 되어 있을 경우를 대비

# ==========================================
# 1. Core Engineering Logic (동적 엔지니어링 엔진)
# ==========================================
        ...
        with st.chat_message("assistant"):
            with st.spinner("AI Agent가 현장 데이터 분석 및 경제적 타당성(VE)을 검토 중입니다..."):
                try:
                    ...
                    if sld_file:
                        if sld_file.type == "application/pdf":
                            contents_to_send.append(types.Part.from_bytes(data=sld_file.getvalue(), mime_type='application/pdf'))
                        else:
                            sld_image = Image.open(sld_file)
                        ...
                        try:
                            ...
                            if "503" in error_msg or "429" in error_msg:
                                if attempt < max_retries - 1:
                                    st.warning(f"⚠️ 서버 일시적 혼잡. {retry_delay}초 후 자동으로 재시도합니다...")
                                    time.sleep(retry_delay)
                                    retry_delay *= 2
                                else:
                                    raise Exception("현재 이용자가 많아 서버가 혼잡합니다. 잠시 후 새로고침해주세요.")
                            else:
                                raise api_e
